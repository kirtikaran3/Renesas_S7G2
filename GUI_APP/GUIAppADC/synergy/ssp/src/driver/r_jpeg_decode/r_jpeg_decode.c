 ...
 **********************************************************************************************************************/
ssp_err_t R_JPEG_Decode_OutputBufferSet (jpeg_decode_ctrl_t * p_ctrl, void * p_output_buffer, uint32_t output_buffer_size)
{
    ...
    if (0 == (JPEG_DECODE_STATUS_IMAGE_SIZE_READY & p_ctrl->status))
    {
        ...
        HW_JPEG_DecodeDestinationAddressSet((uint32_t) p_output_buffer);
...
void jpeg_jedi_isr (void)
{
    ...
    if (intertype & (JPEG_INTE0_INS3 | JPEG_INTE0_INS5 | JPEG_INTE0_INS6))
    {
        /** Clear the request.  */
