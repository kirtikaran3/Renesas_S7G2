...
static          void        riic_rxi_master         (uint32_t   const           channel)
{
    ...
    if (1 != p_transfer->loaded)
    {
#if (DEBUG)
        *p_seq++ = g_RXFULL_ISR_first_byte_dummy_read_str;
#endif

        /* Do this sequence for one byte receive, as per HW manual*/
        ...
        /* On the first interrupt issue a dummy read, according to the manual */
    ...
    {
        /* Set the wait bit before reading the second to last byte, according to the manual */
        if (2 == p_transfer->remain)
        {
#if (DEBUG)
            *p_seq++ = g_RXFULL_ISR_2_byte_left_set_wait_str;
#endif
            HW_RIIC_SetWait(channel);
        }
#if (DEBUG)
        *p_seq++ = g_RXFULL_ISR_read_to_buff_str;
#endif
        uint32_t current = p_transfer->total;
        current -= p_transfer->remain;
        p_transfer->p_buff[current] = HW_RIIC_ReadByte(channel);

        /* Update the number of bytes received */
        p_transfer->remain--;
    }

    /* Only notify when there were no errors during processing */
    if (!p_transfer->err)
    {
#if (DEBUG)
        *p_seq++ = g_RXFULL_ISR_no_errors_str;
#endif
        if (0 == p_transfer->remain)
        {
#if (DEBUG)
            *p_seq++ = g_RXFULL_ISR_no_bytes_left_str;
#endif
            /* Notify anyone waiting that the transfer is finished */
            //MT modified riic_notify(p_transfer, I2C_EVENT_RX_COMPLETE);

            /* Set the bit that will end the data transfer now, according to the manual */
            if (p_transfer->restart)
            {
#if (DEBUG)
                *p_seq++ = g_RXFULL_ISR_send_restart_str;
#endif
            ...
            {
#if (DEBUG)
                *p_seq++ = g_RXFULL_ISR_send_stop_str;
#endif
        ...
        {
            /* Set the ack bit before the last byte, according to the manual */
            if (1 == p_transfer->remain)
            {
#if (DEBUG)
                *p_seq++ = g_RXFULL_ISR_last_byte_set_ACK_str;
#endif
                HW_RIIC_SetAck(channel);
            }
        }
