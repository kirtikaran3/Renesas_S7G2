...
static void lcd_write(uint8_t cmd, uint8_t* data ,uint32_t len)
{
    ...
    if (len)
    {
        ...
        err = g_lcd_spi.p_api->write(g_lcd_spi.p_ctrl, data, len,SPI_BIT_WIDTH_8_BITS);
        if (SSP_SUCCESS != err)
        {
            while(1);
        }
...
static void lcd_read(uint8_t cmd, uint8_t* data ,uint32_t len)
{
    ...
    g_ioport.p_api->pinWrite(LCD_CMD,IOPORT_LEVEL_HIGH);
    g_ioport.p_api->pinCfg(IOPORT_PORT_01_PIN_02,(IOPORT_CFG_PORT_DIRECTION_OUTPUT | ((g_lcd_spi.p_cfg->clk_polarity == SPI_CLK_POLARITY_HIGH) ? IOPORT_LEVEL_LOW : IOPORT_LEVEL_HIGH)));
    ...
    err = g_lcd_spi.p_api->writeRead(g_lcd_spi.p_ctrl, dummy_write, data,len,SPI_BIT_WIDTH_8_BITS);
    if (SSP_SUCCESS != err)
    {
        while(1);
    }
