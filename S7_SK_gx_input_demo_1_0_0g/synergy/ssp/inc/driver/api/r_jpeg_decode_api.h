 ...
 * Macro definitions
 **********************************************************************************************************************/
#define JPEG_DECODE_API_VERSION_MAJOR (1)
#define JPEG_DECODE_API_VERSION_MINOR (0)
...
{
    JPEG_DECODE_PIXEL_FORMAT_ARGB8888 = 1,              ///< Pixel Data ARG B888 format
    JPEG_DECODE_PIXEL_FORMAT_RGB565                     ///< Pixel Data RGB 565 format
} jpeg_decode_pixel_format_t;

/** JPEG HLD driver internal status information */
typedef enum e_jpeg_decode_status
{
    JPEG_DECODE_STATUS_FREE                  = 0x0,     ///< JPEG
    ...
    JPEG_DECODE_STATUS_OUTPUT_PAUSE          = 0x10,    ///< JPEG Codec paused after decoded the number of lines
                                                        ///<  specified by user
    ...
    JPEG_DECODE_STATUS_HEADER_PROCESSING     = 0x80     ///< JPEG Codec module encountered an error.
...
{
    JPEG_DECODE_OUTPUT_NO_SUBSAMPLE = 0,                ///< No subsample.  The image is decoded with no reduction in
                                                        // size.
...
/** Data type for decoding count mode enable. */
/* FIXME: This should be removed because we shall always do Reinit? Maybe.. let's review the HW requirement. */
typedef enum e_jpeg_decode_resume_mode
{
    JPEG_DECODE_COUNT_MODE_ADDRESS_CONTINUE = 0,                 ///< The data buffer address will not be initialized
                                                                 // when resuming image data lines.
    JPEG_DECODE_COUNT_MODE_ADDRESS_REINITIALIZE                  ///< The data buffer address will be initialized when
                                                                 // resuming image data lines.
...
typedef struct st_jpeg_decode_cfg
{
    ...
    uint8_t                     alpha_value;        ///< Alpha value to be applied to decoded pixel data.  This only
                                                    // works if the image pixel format is ARGB888 format.
...
typedef struct st_jpeg_decode_ctrl
{
    ...
    uint32_t                    total_lines_decoded; ///< Track the number of lines decoded so far. 
