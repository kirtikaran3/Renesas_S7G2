 ...
 * @defgroup SF_JPEG_DECODE_API JPEG Framework Decode Interface
...
#define SF_JPEG_DECODE_API_VERSION_MAJOR (1)
#define SF_JPEG_DECODE_API_VERSION_MINOR (0)

/**********************************************************************************************************************
 * Typedef definitions
 **********************************************************************************************************************/
/** JPEG framework control block.  DO NOT INITIALIZE.  Initialization occurs when  SF_JPEG_Decode_Open() is called. */
typedef struct st_sf_jpeg_ctrl
{
    ...
    TX_EVENT_FLAGS_GROUP     events;           ///< Event flags used by the HAL driver to notify the framework driver of
                                               // events
    jpeg_decode_api_t const  * p_api;          ///< Pointer to lower level driver function pointers
    jpeg_decode_ctrl_t       ctrl;             ///< Lower level driver control block
} sf_jpeg_decode_ctrl_t;

/** Configuration for RTOS integrated JPEG driver */
typedef struct st_sf_jpeg_cfg
{
    /** Pointer to a driver structure that implements this interface. Pre-configured driver
     *  structures are located in r_jpeg.c and extern'ed in r_jpeg.h. */
    jpeg_decode_api_t const  * p_lower_lvl_api;

    /** Pointer to JPEG configuration parameter structure.   */
    jpeg_decode_cfg_t const  * p_lower_lvl_cfg;
