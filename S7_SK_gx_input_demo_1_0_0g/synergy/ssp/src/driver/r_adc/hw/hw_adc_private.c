...
void HW_ADC_Open(adc_cfg_t const * const p_cfg)
{
    ...
    {
        R_MSTP->MSTPCRD_b.MSTPD16 = 0;      /** ADC 0 power on   */
    }
    else
    {
        R_MSTP->MSTPCRD_b.MSTPD15 = 0;      /** ADC 1 power on   */
    }
    ...
    else if (ADC_TRIGGER_ASYNC_EXT_TRG0 == p_cfg->trigger)
    {
        ...
        p_regs->ADCSR_b.EXTRG = 1;
        /** Set TRSA to 0x00 if using Asynchronous external trigger*/
        p_regs->ADSTRGR_b.TRSA = 0x00;
    }


    /** Configure group scan mode*/
    ...
    p_regs->ADCER_b.ADRFMT = (ADC_ALIGNMENT_LEFT == p_cfg->alignment) ? 1 : 0;
    /** Configure is the result register should be cleared after reading for this unit */
    p_regs->ADCER_b.ACE = (ADC_CLEAR_AFTER_READ_ON == p_cfg->clearing) ? 1 : 0;
...
ssp_err_t HW_ADC_ScanCfg(uint16_t const unit,  adc_channel_cfg_t const * const p_cfg)
{
    ...
    /** Set mask for channels and sensors for channels 0-15  */
    p_regs->ADANSA0 = (uint16_t) (p_cfg->scan_mask & 0x0000FFFF);
    p_regs->ADANSB0 = (uint16_t) (p_cfg->scan_mask_group_b & 0x0000FFFF);
    p_regs->ADADS0 = (uint16_t) (p_cfg->add_mask & 0x0000FFFF);
    ...
    p_regs->ADANSA1 = (uint16_t) ((p_cfg->scan_mask >> 16) & 0x0000003F);
    p_regs->ADANSB1 = (uint16_t) ((p_cfg->scan_mask_group_b >> 16) & 0x0000003F);
    p_regs->ADADS1 = (uint16_t) ((p_cfg->add_mask >> 16) & 0x0000003F);
...
ssp_err_t HW_ADC_ScanCfgCheck(uint16_t const unit, adc_channel_cfg_t const * const p_cfg)
{
    ...
    if ((0 == p_cfg->scan_mask)
     || (0 != (p_cfg->scan_mask & ADC_INVALID_CH_MASK(unit))))
    {
 ...
 /** Verify at least one unique bonded channel is selected for Group B
    ...
    if (ADC_ADCS_GROUP_SCAN == p_regs->ADCSR_b.ADCS)
    {
        if ((0 == p_cfg->scan_mask_group_b)
         || (0 != (p_cfg->scan_mask_group_b & ADC_INVALID_CH_MASK(unit))))
        {
            return SSP_ERR_INVALID_ARGUMENT;
        }
...
void HW_ADC_SensorCfg(uint16_t const unit, adc_channel_cfg_t const * const p_cfg)
{
    ...
    if ((p_cfg->scan_mask & ADC_MASK_VOLT) || (p_cfg->scan_mask_group_b & ADC_MASK_VOLT))
    {
        ...
        p_regs->ADEXICR_b.OCSAD = (p_cfg->add_mask & ADC_MASK_VOLT) ? 1 : 0; /** Enable voltage addition mode if set */
    }
