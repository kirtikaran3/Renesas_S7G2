...
#include "./hw/target/s124/hw_adc_private_s124.h"
#endif
/**********************************************************************************************************************
 * Macro definitions
 **********************************************************************************************************************/
...
ssp_err_t R_ADC_Open(adc_ctrl_t * p_ctrl,  adc_cfg_t const * const p_cfg)
{
    ...
    bsp_hw_lock_t lock_type = ADC_LOCK(p_cfg->unit);
    if (SSP_ERR_IN_USE == R_BSP_HardwareLock(lock_type))
    {
...
ssp_err_t R_ADC_InfoGet(adc_ctrl_t * p_ctrl, adc_info_t * p_adc_info)
{
    ...
    HW_ADC_ELCEventGet(p_ctrl->unit, p_adc_info);
...
ssp_err_t R_ADC_Read(adc_ctrl_t * p_ctrl, adc_register_t const  reg_id, adc_data_size_t * const p_data)
{
    ...
    arr = (dregs_t *)GET_DATA_ARR(p_ctrl->unit);
    if ((NULL == arr[reg_id]) ||(reg_id > ADC_REG_VOLT))
    {
        return SSP_ERR_INVALID_ARGUMENT;
    }
#else
...
ssp_err_t R_ADC_Close(adc_ctrl_t * p_ctrl)
{
    ...
    /** Release the lock*/
    bsp_hw_lock_t lock_type = ADC_LOCK(p_ctrl->unit);
    R_BSP_HardwareUnlock(lock_type);
...
static ssp_err_t r_adc_open_cfg_check(uint16_t unit, adc_mode_t const  mode, adc_cfg_t const * const p_cfg)
{
    ...
    if ((p_cfg->p_callback != NULL))
    {
        ...
        if (ADC_UNIT_0 == unit)
        {
            ...
            if((uint32_t)BSP_IRQ_CFG_ADC0_SCAN_END == BSP_IRQ_DISABLED)
            {
        ...
        else if (ADC_UNIT_1 == unit)
        {
            ...
            if((uint32_t)BSP_IRQ_CFG_ADC1_SCAN_END == BSP_IRQ_DISABLED)
            {
                err = SSP_ERR_IRQ_BSP_DISABLED;
            }
#endif
