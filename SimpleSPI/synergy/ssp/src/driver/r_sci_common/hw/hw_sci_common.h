...
* @note      All the parameter check must be held by HLD
***********************************************************************************************************************/
__STATIC_INLINE void HW_SCI_RegisterReset (uint32_t const channel)
{
    SCI_REG(channel)->SMR = 0U;
    SCI_REG(channel)->SCR = 0U;
    SCI_REG(channel)->SSR = 0U;
    SCI_REG(channel)->SCMR = 0xF2U;
    SCI_REG(channel)->BRR = 0xFFU;
    SCI_REG(channel)->MDDR = 0xFFU;
    SCI_REG(channel)->SEMR = 0U;
    SCI_REG(channel)->SNFR = 0U;
    SCI_REG(channel)->SIMR1 = 0U;
    SCI_REG(channel)->SIMR2 = 0U;
    SCI_REG(channel)->SIMR3 = 0U;
    SCI_REG(channel)->SISR = 0U;
    SCI_REG(channel)->SPMR = 0U;
    SCI_REG(channel)->SISR = 0U;
    SCI_REG(channel)->FCR = 0xF800U;
    SCI_REG(channel)->CDR = 0U;
    SCI_REG(channel)->DCCR = 0x40U;
    SCI_REG(channel)->SPTR = 0x03U;
