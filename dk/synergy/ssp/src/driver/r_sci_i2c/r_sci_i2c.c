...
static void sci_siic_sda_delay_settings(uint8_t const  clk_divisor,
                                               uint16_t  sda_delay,
                                         ...
                                         uint8_t  * const   p_divisor)
{
    ...
    /*  In order to get the standard 300ns SDA output delay for S7 devices, we should keep the clock divisor (CKS)
     *  PCLK/4 clock (ie, n = 1), so start the index from 1.
     *  Otherwise with PCLKA at 120MHz (max frequency on an S7 device for PCLKA), and CKS[1:0] bits in SMR = 0 (PCLKA/1)
     *  it is not possible to set the required 300ns delay (max delay is 31/120MHz = 258ns), other devices can achieve this delay with PCLKA/1.
     *  */
    for (i = 1; i < SCI_SIIC_NUM_DIVISORS_SYNC; i++)
#else
 ...
 * @param[in]  sda_delay     Requested SDA delay in nano seconds. Standard value is 300ns and Maximum supported value is 1000ns.
 * @param[out] cycles value  value to be set in IICDL bits
 ********************************************************************************************************************/
static void sci_siic_sda_delay_settings(uint8_t const  clk_divisor,
                                         uint16_t      sda_delay,
                                         uint8_t  * const   p_cycles)
{
    uint32_t clk_divisor_value=0;
    float    freq = 0.0;
    uint32_t clock_mhz;

#if (BSP_CFG_MCU_PART_SERIES == 3)
    /* S3A7 devices shows a default 300ns approx SDA delay, so adjust that delay while calculating new delay */
    if(sda_delay < 300)
    {
        sda_delay = 0;
    }
    ...
    {
        sda_delay = (uint16_t)(sda_delay - 300);
    }
#endif

    clk_divisor_value = (uint8_t)(1 << (clk_divisor * 2));
    ...
    freq  = (float)(clock_mhz/(clk_divisor_value * 1000000)) / 1000;

    *p_cycles = (uint8_t)(freq * sda_delay);
}
...
static ssp_err_t sci_siic_open_hw_master(i2c_ctrl_t * const p_ctrl)
{
    ...
    if (do_full_cfg)
    {
        ...
        HW_SCI_RegisterReset(p_ctrl->info.channel);
    ...
    if (do_clock_cfg)
    {
        ...
        sci_siic_sda_delay_settings(clk_divisor, p_ctrl->info.sda_delay, &cycles);
        HW_SCI_SIIC_DataOutputDelay (p_ctrl->info.channel, cycles);
    }
